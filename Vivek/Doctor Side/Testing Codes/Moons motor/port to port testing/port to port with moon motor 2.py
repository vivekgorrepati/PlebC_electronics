...
def move(ser, target_position):
            ...
            # direction = 'FL' if steps > 0 else 'BL'
            # send(ser, f'{direction}{abs(steps)}')  # Move forward (FL) or backward (BL) by the specified number of steps
            ...
            if ser.isOpen():
                try:
                    ...
                    move(ser, 100)    # Move forward by 100 steps (You can adjust the steps as needed)
...
while True:
    ...
    if len(values) > 4:
        ...
        if bestangles != "no solution":
            ...
            # current_position += bestangles[0]  # Assuming bestangles[0] represents the movement in this context
            target_position = (bestangles[0] * (200000/360))
            print(bestangles[0])
