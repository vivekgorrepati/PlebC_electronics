...
//HAL_StatusTypeDef BNO055_ReadCalibStatus(uint8_t *calib_status) {
//    return BNO055_ReadRegs(BNO055_CALIB_STAT_REG, calib_status, 1);
//}
//
//HAL_StatusTypeDef BNO055_ReadAccel(int16_t *acc_x, int16_t *acc_y, int16_t *acc_z) {
//    uint8_t buffer[6];
//    HAL_StatusTypeDef ret;
//
//    ret = BNO055_ReadRegs(BNO055_ACC_DATA_X_LSB, buffer, 6);
//    if (ret != HAL_OK) return ret;
//
...
//	  if (BNO055_ReadCalibStatus(&calib_status) == HAL_OK) {
//	   printf("Calibration Status: %d\n", calib_status);
//	   } else {
//		   printf("Failed to read calibration status\n");
//	   }
//
//	   if (BNO055_ReadAccel(&acc_x, &acc_y, &acc_z) == HAL_OK) {
